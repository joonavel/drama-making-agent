...
class StoryBible(BaseModel):
    ...
    toneDescription: str = Field(description="dramatic, comedic, noir, romantic, thriller 등 톤 설명")
    ...
    negativePrompt: str = Field(description="the elements that should not appear in the frame")
...
class EmotionalArc(BaseModel):
    ...
    """
    startEmotion: str = Field(description="Initial emotion of the character in the story")
...
class DialogueVoice(BaseModel):
    ...
    """
    formality: Literal["formal", "casual", "mixed"] = Field(description="the level of formality of the dialogue")
    accentHints: Optional[str] = Field(default=None, description="the hints of the accent/dialect/speech features")
...
class CharacterBible(BaseModel):
    ...
    """
    CHARACTERINFO: List[CharacterInfo] = Field(description="the list of character information")
...
class StyleBible(BaseModel):
    ...
    """
    cinematography: str = Field(description="the preferred cinematography(handheld/static/tracking etc.)")
    colorPalette: List[HexColor] = Field(
        description="the representative colors(HEX)"
    )
    lighting: str = Field(description="the preferred lighting(high-key/low-key/dramatic/natural etc.)")
    filmGrain: Literal["none", "subtle", "prominent"] = Field(description="the degree of film grain")
    artisticStyle: Literal[
        "photorealistic", "cinematic", "stylized", "anime-inspired"
    ] = Field(description="the artistic style")
    negativeVisualElements: List[str] = Field(description="the visual elements that should not appear in the frame")
...
class Dialogue(BaseModel):
    ...
    emotion: str = Field(description="Emotional state (e.g., 'Angry', 'Whispering', 'Joyful')")
...
class KeyframeDescription(BaseModel):
    ...
    timecode: float = Field(description="Timeline position in seconds (e.g., 0.0, 8.0, 16.0, 24.0, 32.0)")
...
class ShotPlan(BaseModel):
    ...
    camera_movement: str = Field(description="Camera movement description (e.g., 'Slow dolly in', 'Tracking shot')")
    character_action: str = Field(description="Action description between the keyframes")
...
class KeyframePrompt(BaseModel):
    ...
    prompt: str = Field(description="Optimized English prompt for Image Generation Model")
